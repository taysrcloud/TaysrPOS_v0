...
const App = () => {
  ...
  // transactionTabs/currentTransactions/currentTransactionsTotal/currentTransactionsDue/
  // latestSuspendedSale moved to RegisterPage.tsx - confirmed register-exclusive (only
  // consumer was the Transactions modal inside renderRegister). See TRACE.md.
  ...
  // renderRegister (POS cart/checkout) extracted to src/pages/RegisterPage.tsx (Phase 1 registry work, 2026-08-13). See TRACE.md for the dependency audit.
...
const SettingsWarrantiesTab = () => {
  ...
  const load = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const items = data.warranties || data;
        setWarranties(Array.isArray(items) ? items : []);
      }
    } catch { setWarranties([]); }
  };
  useEffect(() => { void load(); }, []);
      ...
      <div className="cart-table">
        ...
        {warranties.map(w => (
          <div className="cart-row" key={w.id} style={{ gridTemplateColumns: '2fr 1fr 1fr' }}>
            ...
            <span>
              <button className="ghost-action" onClick={async () => { await apiFetch(`/api/warranties/${w.id}`, { method: 'DELETE' }); await load(); }} style={{ color: '#ef4444' }}>Supprimer</button>
            </span>
          </div>
        ))}
      ...
      <div className="cart-table">
        ...
        {templates.map(t => (
          <div className="cart-row" key={t.id} style={{ gridTemplateColumns: '1fr 2fr 1fr' }}>
            ...
            <span>
              <button className="ghost-action" onClick={async () => { await apiFetch(`/api/variation-templates/${t.id}`, { method: 'DELETE' }); await load(); }} style={{ color: '#ef4444' }}>Supprimer</button>
            </span>
          </div>
        ))}
...
const SettingsDiscountsTab = () => {
  ...
  const load = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const items = data.discounts || data;
    ...
    <div className="panel" style={{ padding: '2rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem', maxWidth: '600px' }}>
        ...
        <button className="primary-action" onClick={async () => { 
          ...
          await load(); 
        }} style={{ alignSelf: 'flex-start' }}>Créer la promotion</button>
      </div>
      <div className="cart-table">
        ...
        {discounts.map(d => (
          <div className="cart-row" key={d.id} style={{ gridTemplateColumns: '2fr 1fr 1fr 1fr' }}>
            ...
            <span>
              <button className="ghost-action" onClick={async () => { await apiFetch(`/api/discounts/${d.id}`, { method: 'DELETE' }); await load(); }} style={{ color: '#ef4444' }}>Supprimer</button>
            </span>
          </div>
        ))}
      ...
      <div className="cart-table">
        ...
        {devices.map(d => (
          <div className="cart-row" key={d.id} style={{ gridTemplateColumns: '2fr 1fr 1fr' }}>
            ...
            <span>
              <button className="ghost-action" onClick={async () => { await apiFetch(`/api/settings/devices/${d.id}`, { method: 'DELETE' }); await load(); }} style={{ color: '#ef4444' }}>Révoker</button>
            </span>
          </div>
        ))}
